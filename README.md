...
- `媒体库/视图/` 下的图书、电视剧、电影、动漫和游戏 5 个 `.base` 文件
- `媒体库/模板/手动/` 下的 5 个作品录入模板（不含 `合集.md`）
- `媒体库/模板/豆瓣/` 下的 4 个导入模板

如果曾经改过首页、视图、模板或 CSS，请对比合并，否则覆盖会丢失个人定制。

### 2. 处理 Obsidian 和 QuickAdd 配置

- 没有自定义属性类型时，可覆盖 `.obsidian/types.json`。已有自定义时，只需在原文件的 `types` 对象中加入 `"started_at": "date"` 和 `"finished_at": "date"`。
...
不要从新版本复制或覆盖 `媒体库/作品/`、`媒体库/合集/` 和 `媒体库/附件/`，新版本中的虚构示例也不需要复制。

旧作品没有 `started_at` 或 `finished_at` 也能正常使用，升级不会批量猜测或改写历史日期：
