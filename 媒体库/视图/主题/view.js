...
const MEDIA_TYPE_CONTROLLER_VERSION = 5;
  ...
  const controller = {
    ...
    async launch(typeId) {
      ...
      if (type.flexibleFormat) {
        // QuickAdd can keep an older in-memory copy of data.json until restart.
        // Update both bundled Douban actions before it next saves settings.
        try {
          const quickAdd = app.plugins?.plugins?.quickadd;
          let changed = false;
          for (const flexibleType of MEDIA_TYPE_DEFAULTS.filter(item => item.flexibleFormat)) {
      ...
      if (!app.commands.findCommand?.(commandId)) {
        try {
          const quickAdd = app.plugins?.plugins?.quickadd;
          if (quickAdd?.loadSettings) {
            await quickAdd.loadSettings();
            ...
            } else if (quickAdd.getChoiceById && quickAdd.addCommandForChoice) {
      ...
      if (!app.commands.findCommand?.(commandId)) {
        try {
          ...
          if (NoticeClass) new NoticeClass(`无法打开“新增${type.label}”，请确认 QuickAdd 已启用`);
        } catch (error) {
        }
        return false;
      }
